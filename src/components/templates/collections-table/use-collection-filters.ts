...
type CollectionDateFilter = null | {
  gt?: string
  lt?: string
}

type CollectionFilterAction =
  | { type: "setQuery"; payload: string | null }
  | { type: "setFilters"; payload: CollectionFilterState }
  | { type: "reset"; payload: CollectionFilterState }
  | { type: "setOffset"; payload: number }
  | { type: "setDefaults"; payload: CollectionDefaultFilters | null }
  | { type: "setDate"; payload: CollectionDateFilter }
...
interface CollectionFilterState {
  ...
  limit: number
  offset: number
  additionalFilters: CollectionDefaultFilters | null
}

const allowedFilters = ["updated_at", "created_at", "q", "offset", "limit"]
...
const formatDateFilter = (filter: CollectionDateFilter) => {
  if (filter === null) {
    return filter
  }
...
export const useCollectionFilters = (
  existing?: string,
  defaultFilters: CollectionDefaultFilters | null = null
) => {
  ...
  const setDateFilter = (filter: CollectionDateFilter | null) => {
  ...
  const setDefaultFilters = (filters: CollectionDefaultFilters | null) => {
  ...
  const setFilters = (filters: CollectionFilterState) => {
      ...
      if (key === "query") {
        ...
        if (key === "date") {
          toQuery[stateFilterMap[key]] = formatDateFilter(
            value.filter as CollectionDateFilter
          )
        } else {
  ...
  const getRepresentationObject = (fromObject?: CollectionFilterState) => {
  ...
  const saveTab = (tabName: string, filters: CollectionFilterState) => {
...
const parseQueryString = (
  queryString?: string,
  additionals: CollectionDefaultFilters | null = null
): CollectionFilterState => {
  const defaultVal: CollectionFilterState = {
    status: {
      open: false,
      filter: null,
    },
