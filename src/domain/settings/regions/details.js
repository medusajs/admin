...
const Regions = ({ id }) => {
	...
	useEffect(() => {
		...
		setFulfillmentOptions(
			store.fulfillment_providers.map((c) =>
				fulfillmentProvidersMapper(c.id)
			)
		);
	...
	useEffect(() => {
		...
		setPaymentProviders(
			region.payment_providers.map((v) => ({ value: v.id }))
		);
		...
		<Flex flexDirection="column" py={5} mb={5}>
			...
			<Card as="form" mb={3} onSubmit={handleSubmit(onSave)}>
				...
				<Card.Footer px={3} justifyContent="flex-end">
					...
					<Button variant="danger" onClick={onDelete} mx={2}>
						Delete
					</Button>
				</Card.Footer>
			</Card>
			{!isLoading && (
				<Shipping
					region={region}
					fulfillmentMethods={fulfillmentEndpoint}
				/>
			)}
		</Flex>
	);
